...
frappe.ui.form.on("Sales Confirmation", {
  ...
  add_custom_buttons: function (frm) {
    if (frm.doc.confirmation_date) {
              ...
              if (r.message) {
                ...
                setTimeout(() => {
                  frm.dashboard.set_headline(
                    __(
                      'Please check Invalid Items file <a href="{0}"></a> for errors. ',
                      r.message
                    ),
                    "red"
                  );
      ...
      frm.add_custom_button(
        __("Update Item/PO Item"),
        function () {
          frappe.confirm(
            __("Do you want to Update Items and Purchase Order?"),
            function () {
              return frm
                .call({
    ...
    let fieldname = "sales_confirmation_detail",
      grid = frm.fields_dict[fieldname].grid;
    grid.wrapper
      .find(".grid-upload")
      .removeClass("hidden")
      .off("click")
        ...
        new frappe.ui.FileUploader({
          ...
          on_success(file) {
            ...
            reader.onload = function (e) {
              ...
              console.log(items[0]);
              items.forEach((xl_item) => {
                frm.doc.sales_confirmation_detail
                  .filter((d) => {
                    return d.item_code == xl_item["Your item number"];
                  })
                  .forEach((d) => {
                    update_item(xl_item, d);
                  });
              });
  ...
  supplier_email_callback: function (frm) {
    ...
    setTimeout(() => {
      frappe.call({
        method:
          "art_collections.art_collections.doctype.sales_confirmation.sales_confirmation.supplier_email_callback",
        args: {
          docname: frm.doc.name,
        },
        callback: function (r) {
...
function update_item(excel_item, cdn) {
  const field_map = {
    "Our item number": "supplier_part_no",
    Gencode: "barcode",
    "English description": "supplier_item_description_ar",
    "French description": "item_name",
    Packaging: "packing_type_art",
    "Qty in packaging": "qty_in_selling_pack_art",
    "Qty per inner": "qty_per_inner",
    "Qty per outer": "qty_per_outer",
    "Qty ordered": "qty",
    "Unit price (in $)": "rate",
    "Total of cartons": "total_outer_cartons_art",
    "Each CBM": "cbm_per_outer_art",
    "Total CBM": "total_cbm",
    "HS CODE": "customs_tariff_number",
  };

  for (const key in field_map) {
    const field_name = field_map[key];
    console.log(field_name, excel_item[key]);
    frappe.model.set_value(cdn.doctype, cdn.name, field_name, excel_item[key]);
  }
}
