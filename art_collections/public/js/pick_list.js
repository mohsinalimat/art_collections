frappe.ui.form.on("Pick List", {
  ...
  add_custom_buttons: (frm) => {
    ...
    if (
      ...
      frm.doc.docstatus == 0 &&
      frm.doc.locations != undefined &&
      frm.doc.locations.length > 1
    ) {
      frm.add_custom_button(__("Optimize Path"), () =>
        frm.events.sort_as_per_warehouse(frm)
      );
    }
    // create insufficent items excel

    frm.add_custom_button(
      __("Insufficient Items Excel"),
      () => {
        frappe.call({
          method:
            "art_collections.pick_list_controller.create_insufficient_items",
  ...
  setup: (frm) => {
    frappe.db
      .get_single_value("Art Collections Settings", "picker_role")
      .then((picker_role) => {
        ...
        if (picker_role) {
          frm.set_query("picker_responsible_art", () => {
            return {
              query:
                "art_collections.pick_list_controller.get_user_with_picker_role",
            };
          });
        }
      });
