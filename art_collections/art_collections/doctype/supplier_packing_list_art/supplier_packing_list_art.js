                ...
                // $(this.wrapper).find(".grid-upload").removeClass('hidden').on("click", () => {
                    new frappe.ui.FileUploader({
                        ...
                        on_success(file) {
                            ...
                            $.each(data, (i, row) => {
                                if (i > 6) {
                                    let result={"excel_id":i+1,"action":"","message":""}
                                    var blank_row = true;
                            ...
                            if (excel_uploaded_data.length>0) {
                                ...
                                frm.call({
                                    method: 'validate_excel_uploaded_data',
                                    doc: frm.doc,
                                    args: {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus == 0) {
				frm.add_custom_button(__('Purchase Order'),
					function () {
						// for (let index = 0; index < frm.doc.supplier_packing_list_detail.length; index++) {
						// 	const element = frm.doc.supplier_packing_list_detail[index];
						// 	if (!element.item_code) {
						// 		frm.get_field('supplier_packing_list_detail').grid.grid_rows[index].remove()
						// 	}
