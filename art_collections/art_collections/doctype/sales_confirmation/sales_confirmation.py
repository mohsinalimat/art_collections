    ...
    def update_items_and_po(self):
        ...
        for d in self.sales_confirmation_detail:
            ...
            for poi in [x for x in po.items if x.item_code == d.item_code]:
                # if not cint(d.checked): continue
                for field in [
                    ...
                    poi[field] = d.get(field)
        self.db_set("status", "Validated")

    @frappe.whitelist()
    def validate_with_po(self):
        invalid_items = []
        po = frappe.get_doc("Purchase Order", self.purchase_order)
        ...
        if invalid_items:
            ...
            frappe.log_error(message="", title="PO/Sales Confirmation mismatch")
            frappe.throw(
                _("Mismatch in values for items: {}. Please check error log {}").format(
                    ", ".join(x[0].item_code for x in invalid_items), "/"
                )
            )
...
def get_items_xlsx(docname):
    ...
    print(frappe.db.get_description())
