...
territory.default_payment_terms_template_cf,territory.minimum_order_amount_cf,customer_credit_limit.credit_limit,customer_credit_limit.company 
...
where territory.name=%s""",(territory), as_dict=1,debug=1)
	if len(territory) >0:
...
from `tabTerritory` territory left outer join `tabCustomer Credit Limit` customer_credit_limit on customer_credit_limit.parent = territory.credit_limit_cf 
and customer_credit_limit.parenttype='Territory' and customer_credit_limit.idx=1
where territory.lft <= %s and territory.rgt >= %s and territory.is_group=1 order by territory.lft desc limit 1""",(territory[0].lft,territory[0].rgt), as_dict=1,debug=1)	
			if len(territory) >0:
				return territory[0]
			else:
				None		
